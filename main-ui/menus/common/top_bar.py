...
class TopBar:
    ...
    def render_top_bar(self, title) :
        ...
        battery_text_w, battery_text_h = Display.render_text(str(battery_percent),self.device.screen_width - padding*2, center_of_bar,  Theme.text_color(FontPurpose.BATTERY_PERCENT), FontPurpose.BATTERY_PERCENT, RenderMode.MIDDLE_RIGHT_ALIGNED)
        ...
        if(Theme.show_top_bar_text()):
            Display.render_text(title,int(self.device.screen_width/2), center_of_bar, Theme.text_color(FontPurpose.TOP_BAR_TEXT), FontPurpose.TOP_BAR_TEXT, RenderMode.MIDDLE_CENTER_ALIGNED)
